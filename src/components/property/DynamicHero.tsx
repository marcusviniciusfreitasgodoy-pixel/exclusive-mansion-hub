...
export const DynamicHero = ({ property, branding }: DynamicHeroProps) => {
  const scrollToDetails = () => {
    document.getElementById("details")?.scrollIntoView({ behavior: "smooth" });
  };

  const heroImage = property.imagens?.[0]?.url || "/placeholder.svg";
  const location = [property.bairro, property.cidade].filter(Boolean).join(", ");

  // Format area for subtitle
  const area = property.areaPrivativa || property.areaTotal;
  const areaText = area ? `${area}m² de Sofisticação` : "Sofisticação";
  const locationText = location ? ` em ${location}` : "";

  // WhatsApp link with imobiliaria phone
      ...
      <div className="relative z-10 flex h-full flex-col items-center justify-center px-6 py-8">
        ...
        <div className="text-center animate-fade-in mt-20">
          <p className="mb-4 text-sm uppercase tracking-[0.3em] text-accent md:text-base">
            Exclusividade {branding.imobiliariaNome}
          </p>
