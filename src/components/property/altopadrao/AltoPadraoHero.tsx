...
export function AltoPadraoHero({ property, branding, onContactClick, onGalleryOpen }: AltoPadraoHeroProps) {
  ...
  return (
    <section className="relative h-[70vh] w-full overflow-hidden">
      ...
      <div className="absolute inset-0">
        ...
        {/* Ocean Blue overlay 50% - Primary 800 */}
      ...
      <div className="absolute left-6 top-6 flex gap-3 z-10">
        {photoCount > 0 && (
          <button
            ...
            className="flex items-center gap-2 rounded-lg bg-black/60 px-3 py-2 text-sm text-white backdrop-blur-sm transition-colors hover:bg-black/80"
          >
        ...
        {videoCount > 0 && (
          <div className="flex items-center gap-2 rounded-lg bg-black/60 px-3 py-2 text-sm text-white backdrop-blur-sm">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center px-6 text-center">
        {/* Property Title - Montserrat bold, Primary 500 */}
        <h1
        ...
        {/* CTA Button - Primary 500, border-radius 12px */}
        <Button
          onClick={onContactClick}
          className="px-8 py-3 text-sm font-semibold uppercase tracking-wide transition-all duration-300 hover:shadow-lg"
          style={{
          ...
          {images.map((_, index) => (
            <button
              ...
              className={`h-2 w-2 rounded-full transition-all ${
                index === currentIndex ? "w-6 bg-[#22c55e]" : "bg-white/50"
              }`}
